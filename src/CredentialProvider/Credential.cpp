...
HRESULT DoweCredential::GetSerialization(CREDENTIAL_PROVIDER_GET_SERIALIZATION_RESPONSE* response,CREDENTIAL_PROVIDER_CREDENTIAL_SERIALIZATION* serialization,PWSTR* status,CREDENTIAL_PROVIDER_STATUS_ICON* icon){
 ...
 dowe::ipc::Request q{};dowe::ipc::InitializeRequest(q,dowe::ipc::RequestType::Validate);wcsncpy_s(q.account.data(),q.account.size(),qualifiedUser_.c_str(),_TRUNCATE);wcsncpy_s(q.code.data(),q.code.size(),code_.c_str(),_TRUNCATE);dowe::ipc::Response r{};
